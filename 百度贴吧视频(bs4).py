...
for i in range(5):
    ...
    div = bs.find_all('div',class_ = 'threadlist_video')

    for infos in div:
        a = infos.find('a')
        video = a.get('data-video')if a else None
        # print(video)
        response = requests.get(video,headers = headers)

        with open('./bs4百度贴吧视频/'+str(count)+'.mp4','wb') as fp:
